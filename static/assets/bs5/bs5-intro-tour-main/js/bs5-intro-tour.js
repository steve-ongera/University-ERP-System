function Tour(steps, options) {
    ...
    var getPopoverSettings = function(arrows) {
        ...
        template += '<div class="tour-exit text-end"><i class="ri-close-line "></i></div>' +
            '<h3 class="popover-header"></h3>' +
            '<div class="popover-body">' +
            '</div>' +
            '<div class="popover-footer d-flex justify-content-end">' +
            '   <a class="btn btn-secondary tour-prev me-2">' +
            translations.previous +
            '   </a>' +
            '   <a class="btn tour-next primary-btn me-2">' +
            translations.next +
            '   </a>' +
            '   <a class="btn primary-btn tour-finish hidden">' +
            translations.finish +
            '   </a>' +
            '</div>' +
            '</div>';
...
Tour.prototype.show = function() {
    ...
    const reloadVA = document.querySelector('.reload-VA');
    e.btnFinish.addEventListener("click", function() {
         
        onExit();
        if (reloadVA)
            reloadVA.classList.add('show')
        sessionStorage.setItem('activeMenu', 'virtual')
        location.href = '/virtualassistant/index'
    });
